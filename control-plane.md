...
**[PROPOSAL]** Configure tool policy for each unattended worker, not only for the chat bot: OMP's normal `yolo` mode auto-approves execution tools, and `--no-ui` resolves extension dialogs headlessly rather than supplying a human approver. An approval prompt is not an OS sandbox; tool rules cannot confine an already permitted shell or `eval` process. Give the worker a restricted filesystem/network identity and place approval at the specific privileged action when human intervention is required ([OMP approval modes and limits](https://github.com/can1357/oh-my-pi/blob/main/docs/approval-mode.md), [RPC `--no-ui`](https://github.com/can1357/oh-my-pi/blob/main/docs/rpc.md#startup)).
